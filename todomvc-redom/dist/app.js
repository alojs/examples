 ...
 */
var thunkMiddleware = alo.extras.middlewares.createThunk();
...
var routerStore = alo.createStore(initialState$1, 'router');
 ...
 * Route reducer
 */
var routeReducer = routerStore.createReducer(function (state, action) {
  switch (action.type) {
    case 'setRoute':
...
var css = {
    "hidden": "mc87069c92_hidden",
    "todoapp": "mc87069c92_todoapp",
    "new-todo": "mc87069c92_new-todo",
    "edit": "mc87069c92_edit",
    "main": "mc87069c92_main",
    "toggle-all": "mc87069c92_toggle-all",
    "todo-list": "mc87069c92_todo-list",
    "editing": "mc87069c92_editing",
    "view": "mc87069c92_view",
    "toggle": "mc87069c92_toggle",
    "completed": "mc87069c92_completed",
    "destroy": "mc87069c92_destroy",
    "footer": "mc87069c92_footer",
    "todo-count": "mc87069c92_todo-count",
    "filters": "mc87069c92_filters",
    "selected": "mc87069c92_selected",
    "clear-completed": "mc87069c92_clear-completed",
    "info": "mc87069c92_info"
};
...
var todomvcStyles$1 = todomvc;

var todosStore = alo.createStore([], 'todos');

todosStore.createComputedProperty('notCompleteCount', function(state) {
  var count = 0;

  lodash.each(state, function(todo           ) {
    if (!todo.complete) {
      count++;
    }
  });
...
todosStore.createReducer(function(state, action) {
  switch(action.type) {
    case 'addTodo':
      action.payload.id = action.payload.id || lodash.uniqueId();
      state.push(action.payload);
      break
    case 'removeTodo':
      var id = lodash.findIndex(state, function(todo           ) {
    ...
    case 'removeCompletedTodos':
      state = lodash.filter(state, function(todo           ) {
...
var toggleComplete = function toggleComplete(todo           ) {
  return {
    type: 'toggleComplete',
    payload: todo
  }
...
var setComplete = function setComplete(todo        ) {
  return {
    type: 'setComplete',
    payload: todo
  }
...
var removeCompletedTodos = function removeCompletedTodos() {
...
class Todo {
  ...
  constructor () {
    ...
    this.el = el('li', [
      this.elView = el(`div.${todomvcStyles$1.view}`, [
        this.elToggleInput = el(`input.${todomvcStyles$1.toggle}`, { type: 'checkbox', onclick: function(e) {
        ...
        this.elDestroyBtn = el(`button.${todomvcStyles$1.destroy}`, {
          onclick: function(e) {
            todosStore.dispatch(removeTodo(self.state));
          }
        })
      ]),
      this.elTitleInput = el(`input.${todomvcStyles$1.edit}`, {
        onblur: (e) => {
  ...
  update (todo            ) {
    ...
    this.el.className = cl(
      todomvcStyles$1.editing, this.uiState.editing,
      todomvcStyles$1.completed, !!this.state.complete);
...
class Todos {
  ...
  constructor () {
    var self = this;
    this.list = list(`ul.${todomvcStyles$1['todo-list']}`, Todo, 'id');
    ...
    sub.createDependency({
      ...
      filteredTodos: [['todosFilter', 'todos'], function(stores, computed) {
        switch(computed.todosFilter) {
          case 'active':
            return lodash.filter(computed.todos.state, function(todo           ) {
...
var appStore = alo.createStore(initialState, 'app');
...
var setNewTodoTitle = function(title        ) {
  return {
...
class Footer {
  ...
  update (stores        , computed        ) {
    ...
    this.elFilterList.update(lodash.map(['all', 'active', 'completed'], function(filterName) {
      return {
        name: filterName,
